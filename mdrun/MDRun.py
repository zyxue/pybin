...
class MDRun(object):
    def __init__(self, tpr, **kwargs):
        ...
        self.gmxcheck = str(self.kwargs.get('GMXCHECK', 'gmxcheck'))
        self.gmxdump = str(self.kwargs.get('GMXDUMP', 'gmxdump'))
        self.mdrun_mpi = str(self.kwargs.get('MDRUN_MPI', 'mdrun_mpi'))
    ...
    def _check_integrity(self, inputfile):
        ...
        r = subprocess.call([self.gmxcheck, '-f', inputfile],
    ...
    def _fix_xtc(self, xtc, tpr):
        ...
        r = subprocess.call([self.gmxcheck, '-f', xtc],
                            stdout=pipe, stderr=pipe)
        if r == 0:
            fixable = True
        return fixable
    ...
    def _fix_cpt(self, cpt, prev_cpt):
        ...
        if not os.path.exists(prev_cpt):
            ...
            r_prev_cpt = subprocess.call([self.gmxcheck, '-f', prev_cpt],
                                         stdout=pipe, stderr=pipe)
            if r_prev_cpt != 0:
                return fixable
            else:
                shutil.copy(prev_cpt, cpt)
                r_cpt = subprocess.call([self.gmxcheck, '-f', cpt],
                                        stdout=pipe, stderr=pipe)
                if r_cpt == 0:
                    fixable = True
                return fixable
    ...
    def _cpttime(self, cptfile):
        ...
        stdout, stderr = subprocess.Popen([self.gmxcheck, '-f', cptfile],
    ...
    def _tprtime(self, tprfile):
        stdout, stderr = subprocess.Popen([self.gmxdump, '-s', tprfile],
                                          stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def mdrun(self):
        first_run_command = ['mpirun', '-np', str(self.kwargs.get('NP', 8)), 
